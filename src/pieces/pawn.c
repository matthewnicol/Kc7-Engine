int pawn_moves(Piece piecemap[64], int square, Player turn, Move **moves) 
{
    ...
    blockaded1 = piecemap[square+(8*direction)] != NO_PIECE; 
    // One step moves
    if (!blockaded1 && VALID(square+(8*direction))) 
        moves[i++] = makeMove(makePieceMovement(square, square+(8*direction), piecemap[square], NO_PIECE), blankPieceMovement()); 

    twosquares = RANK_MAP[square] == two_from;
    blockaded2 = !blockaded1 && piecemap[square+(8*direction)] != NO_PIECE;
    // Two step moves
    if (!blockaded2 && twosquares)
        moves[i++] = makeMove(makePieceMovement(square, square+(16*direction), piecemap[square], NO_PIECE), blankPieceMovement());
    ...
    // Add on EN Passant moves if applicable
